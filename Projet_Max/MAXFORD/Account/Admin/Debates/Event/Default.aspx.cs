...
namespace MAXFORD.Account.Admin.Debates.Event
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        protected void AccessDataSource1_Deleting(object sender, SqlDataSourceCommandEventArgs e)
        {
            ...
            //    {
            //        command.CommandText = "SELECT HallImageThumbnail FROM dbo.ClassHall WHERE HallID = @Id";
            //        command.Parameters.AddWithValue("@Id", e.Command.Parameters["@HallID"].Value);
            //        picture1FileName = Convert.ToString(command.ExecuteScalar());
            //    }
            //}
            //if (!string.IsNullOrEmpty(picture1FileName))
            //{
            //    string picture1FilePath_upload = System.IO.Path.Combine(Server.MapPath("~/store/Hall/upload/"), picture1FileName);
            //    if (System.IO.File.Exists(picture1FilePath_upload))
            //    {
            //        System.IO.File.Delete(picture1FilePath_upload);
            //    }

            //    string picture1FilePath_main = System.IO.Path.Combine(Server.MapPath("~/store/Hall/main/"), picture1FileName);
            //    if (System.IO.File.Exists(picture1FilePath_main))
            //    {
            //        System.IO.File.Delete(picture1FilePath_main);
            //    }

            //    string picture1FilePath_thumbnail = System.IO.Path.Combine(Server.MapPath("~/store/Hall/thumbnail/"), picture1FileName);
