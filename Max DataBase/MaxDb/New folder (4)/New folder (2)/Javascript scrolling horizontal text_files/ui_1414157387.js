...
if(!$iTXT.ui.currentAutoPeek){if(!hk.flipped||repeat||hk.flips<limit){if(!hk.flips&&limit>0){hk.flips=0;}
hk.flipped=true;hk.flips++;$iTXT.ui.AutoPeek.display(hk);var afterTime=$itxtUtil.ts()-masterTs;$iTXT.fire("$iTXT:data:log:monitor",{advert:hk.ad,mt:136,mv:hk.initPos.top+','+viewportHeight+','+pageHeight,mv2:afterTime});}}}
...
{node.style.color=color;}}}});$iTXT.ui.Hook.CLICKMODE={CLICKABLE:0,NOT_CLICKABLE:1,TOOLTIP_VISIBLE:2,DELAYED:3};$iTXT.subscribe('$iTXT:contextualizer:drop',function(){$iTXT.SUPPRESS_PBF=true;});$iTXT.subscribe('$iTXT:initialiser:load:complete',VM._.bind($iTXT.ui.HookManager._onNoHook,$iTXT.ui.HookManager));$iTXT.subscribe('$iTXT:initialiser:load:complete',function(){if(!!$iTXT.glob.params.get('js.daw.partners',false)&&!!$iTXT.glob.params.get('js.daw.statschannel',false)&&!$iTXT.core.Browser.isSmartphoneOrTablet()){var triggerpoint=$iTXT.glob.params.getInt('js.daw.trigger',30);var storedheight=document.body.offsetHeight;var minscroll=$iTXT.glob.params.getInt('js.daw.minscroll',900);var minheight=$iTXT.glob.params.getInt('js.daw.minheight',500);var enuf_win=(window.innerHeight||document.documentElement.clientHeight)>minheight;var useScroll=$iTXT.glob.params.getBool('js.daw.usescroll',true);if(!useScroll){minscroll=0;triggerpoint=0;}
function onScrollEnd(){if(storedheight*(triggerpoint/100)<=window.pageYOffset&&window.pageYOffset>=minscroll&&enuf_win){if(!VM.DAW.loaded){var url=$iTXT.glob.params.get('js.daw.url',"http://images.intellitxt.com/ast/js/vm/jslib/resources/daw.js");$iTXT.core.Util.dropScript(url+"?refresh="+(new Date()).getTime());VM.DAW.loaded=true;VM.DAW.showing=true;}else{if(!VM.DAW.showing){VM.DAW.reshow();VM.DAW.showing=true;}}}else{if(VM.DAW.loaded&&VM.DAW.showing){VM.DAW.hide();VM.DAW.showing=false;}}}
